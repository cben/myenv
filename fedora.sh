...
sudo dnf install --setopt=strict=0 \
    ...
    bat git-delta colordiff wdiff git-cinnabar meld gists nano micro emacs sensible-utils \
    ...
    linux-libertine-fonts culmus-\*-fonts levien-inconsolata-fonts fira-code-fonts adobe-source-code-pro-fonts cascadia-fonts-all \
    ...
    openbox obconf xrandr xinput alacritty lxrandr \
    qtile qtile-wayland qtile-extras raindrop xisxwayland vicinae \
    labwc xwayland-run waycheck waypipe grim slurp wf-recorder swaybg swaylock swayimg wob waybar wofi wlr-randr fuzzel \
    mscore
